<?php
  ...
  function getProfile($id) {
    $DB = new DB;
    $bind = array($id);
    $sql = 'SELECT id, fname, surname, university, course, bio FROM Profile where id = ?';
    $result = $DB -> query($sql, $bind);
    $result = $result[0];
    $result['university'] = getUniversity($result['university']);
  ...
  function getSurfaceProfile ($id) {
    ...
    $profile = array($result['id'], $result['fname'], $result['surname']);
    return $profile;
  }
  ...
  function courseAuth($id) {
    $result = getCourse($id);
    if ($result['id']===$_SESSION['user']['course']['id']) {
      return TRUE;
    }
    return FALSE;
  }
  ...
  function getTargetPosts($id, $type, $offset = 0, $limit = 20) {
    ...
    $DB = new DB;
    $bind = array($entity, $limit, $offset);
    $sql = 'SELECT * FROM Post WHERE target = ? LIMIT ?, ? ORDER BY updated DESC';
    $result = $DB -> query($sql, $bind);
